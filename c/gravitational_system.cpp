#include "../h/gravitational_system.hpp"

namespace VeX{

    Gravitational_System::Gravitational_System():
        bodies()
    {}

    void Gravitational_System::update(float delta){
    ...
    void Gravitational_System::draw(float delta){
        for(unsigned int i=0; i<bodies.size(); i++){
            bodies[i]->draw(delta);
        }
    }

    void Gravitational_System::addBody(std::unique_ptr<Circle_Object> newBody){
