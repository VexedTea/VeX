#include "../h/color_gradient.hpp"

namespace VeX{
    
    Color_Gradient::Color_Gradient(const std::vector<sf::Color> & colors):
        colors(colors)
    {
        if(colors.size()==0){
    ...
    sf::Color Color_Gradient::getColorAt(float fraction, bool debug)const{
    ...
    sf::Color Color_Gradient::getFirst(){
        return colors[0];
    }
