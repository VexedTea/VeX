...
namespace VeX{
    ...
    std::shared_ptr<Verlet_Object> Verlet_Solver::addObject(const sf::Vector2f & pos){
        return objects.emplace_back(std::make_shared<Verlet_Object>(pos, objectRadius)); 
    }
    ...
    void Verlet_Solver::setObjectVelocity(std::shared_ptr<Verlet_Object> object, const sf::Vector2f & velocity){
        object->setVelocity(velocity, getStepDeltaTime());
    }

    const std::vector<std::shared_ptr<Verlet_Object>> & Verlet_Solver::getObjects() const {
        return objects;
    }
    ...
    void Verlet_Solver::checkCollisions(){
        ...
        //     std::shared_ptr<Verlet_Object> object1 = objects[i];
        //     for(int j{i + 1}; j < objectCount; j++){
        //         std::shared_ptr<Verlet_Object> object2 = objects[j];
